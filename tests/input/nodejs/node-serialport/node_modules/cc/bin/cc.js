...
deglob(config.files, { ignore: config.ignore }, function (err, files) {
  ...
  if (files.length) {
    var args = ['--linelength', config.linelength];
    if (config.filter.length) {
      args.push('--filter');
      args.push(config.filter.map(function (filter) {
        return '-' + filter;
      }).join(','));
    }
    var cppLintOptions = {
    ...
    pythonShell.run('cpplint.py', cppLintOptions).on('error', function (err) {
