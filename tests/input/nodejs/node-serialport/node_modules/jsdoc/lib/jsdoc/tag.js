...
function parseType(tag, tagDef, meta) {
    try {
        ...
        jsdoc.util.logger.error('Unable to parse a tag\'s type expression%s with tag title "%s" and text "%s": %s', meta.filename ? ' for source file ' + path.join(meta.path, meta.filename) + (meta.lineno ? ' in line ' + meta.lineno : '') : '', tag.originalTitle, tag.text, e.message);
