...
function buildMessage(tagName, meta, desc) {
    var result = format('The @%s tag %s. File: %s, line: %s', tagName, desc, meta.filename, meta.lineno);
...
exports.validate = function (tag, tagDef, meta) {
    ...
    if (!tagDef) {
        ...
        if (!allowUnknownTags || Array.isArray(allowUnknownTags) && allowUnknownTags.indexOf(tag.title) < 0) {
    ...
    // check for minor issues that are usually harmless
            ...
            logger.warn(buildMessage(tag.title, meta, 'does not permit a description; the description will be ignored'));
        }
};