...
function getParentDocletFromEvent(parser, e) {
    if (e.doclet && e.doclet.meta && e.doclet.meta.code && e.doclet.meta.code.node && e.doclet.meta.code.node.parent) {
...
function makeRestParamFinisher() {
    return function (e) {
        ...
        restNode = findRestParam(e.code.node.params || e.code.node.value && e.code.node.value.params || e.code.node.init && e.code.node.init.params || []);
...
function makeDefaultParamFinisher() {
    return function (e) {
        ...
        for (var i = 0, j = 0, l = params.length; i < l; i++) {
            ...
            if (defaultValues[i] && defaultValues[i].right && defaultValues[i].right.type === Syntax.Literal && typeof documentedParams[j].defaultvalue === 'undefined' && defaultValues[i].right.value !== '') {
...
function hasComments(node) {
    return node && node.leadingComments && node.leadingComments.length || node && node.trailingComments && node.trailingComments.length || node && node.innerComments && node.innerComments.length;
}
...
Visitor.prototype.visitNodeComments = function (node, parser, filename) {
    ...
    if (node.type === Syntax.ExpressionStatement && node.directive === 'use strict' && node.trailingComments && node.trailingComments.length) {
