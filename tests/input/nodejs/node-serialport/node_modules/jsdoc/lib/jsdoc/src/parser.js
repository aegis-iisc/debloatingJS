...
function definedInScope(doclet, basename) {
    return Boolean(doclet) && Boolean(doclet.meta) && Boolean(doclet.meta.vars) && Boolean(basename) && hasOwnProp.call(doclet.meta.vars, basename);
}
...
Parser.prototype.astnodeToMemberof = function (node) {
    ...
    // confusing...
    else if (type === Syntax.MethodDefinition && node.parent.parent.parent && node.parent.parent.parent.type === Syntax.ArrowFunctionExpression) {
            doclet = this._getDocletById(node.enclosingScope.nodeId);

            if (doclet) {
                result.memberof = doclet.longname + (node.static === true ? jsdoc.name.SCOPE.PUNC.STATIC : jsdoc.name.SCOPE.PUNC.INSTANCE);
            }
        } else {
...
Parser.prototype.resolveThis = function (node) {
    ...
    if (node.type !== Syntax.Property && node.enclosingScope) {
        ...
        if (node.parent && node.parent.type === Syntax.MethodDefinition && node.parent.kind === 'constructor') {
