...
function moveTrailingComments(source, target, count) {
    if (source.trailingComments) {
        ...
        target.trailingComments = source.trailingComments.slice(source.trailingComments.length - count, count);
...
Walker.prototype._recurse = function (filename, ast) {
    ...
    function logUnknownNodeType(node) {
        logger.debug('Found a node with unrecognized type %s. Ignoring the node and its ' + 'descendants.', node.type);
    }
