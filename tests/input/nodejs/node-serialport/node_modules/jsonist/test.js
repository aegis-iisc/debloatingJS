...
function testServer(serverResponse, statusCode) {
  ...
  function handler(req, res) {
    ...
    res.writeHead(typeof statusCode == 'number' ? statusCode : 200, { 'content-type': 'application/json' });
...
test('server error, non-JSON', function (t) {
  ...
  testServer(responseText, 501).on('ready', function (url) {
...
test('server error, with-JSON', function (t) {
  ...
  testServer(stringify(responseDoc), 501).on('ready', function (url) {
    jsonist.get(url, function (err, data, response) {
      t.notOk(err, 'no error');
