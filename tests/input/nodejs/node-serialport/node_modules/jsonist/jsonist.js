...
function collector(uri, options, callback) {
  ...
  function handle(request) {
    ...
    request.pipe(bl(function (err, data) {
      if (redirect) {
        if (++redirectCount >= (typeof options.followRedirects == 'number' ? options.followRedirects : 10)) return callback(new Error('Response was redirected too many times (' + redirectCount + ')'));
      ...
      if (!data.length) return callback(null, null, request.response);
...
function makeMethod(method, data) {
  function handler(uri, options, callback) {
    ...
    if (_typeof(options.headers) != 'object') options.headers = {};

    if (data && typeof options.headers['content-type'] != 'string') options.headers['content-type'] = 'application/json';

    if (typeof options.headers['accept'] != 'string') options.headers['accept'] = 'application/json';
  ...
  function dataHandler(uri, data, options, callback) {
    ...
    if (typeof data.pipe == 'function') data.pipe(request);else request.end(stringify(data));
    return request;
  }
