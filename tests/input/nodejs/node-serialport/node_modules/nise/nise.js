...
if(listener==null){return;}this.eventListeners=this.eventListeners||{};this.eventListeners[event]=this.eventListeners[event]||[];// 4. If context object’s associated list of event listener
...
if(sinonXhr.supportsTimeout&&_typeof(setInterval.clock)==="object"&&_typeof(Date.clock)==="object"){var initiatedTime=Date.now();var self=this;// Listen to any possible tick by fake timers and check to see if timeout has
...
*/function extend()/* [deep], obj1, obj2, [objn] */{var args=[].slice.call(arguments);var deep=false;if(typeof args[0]=='boolean'){deep=args.shift();}var result=args[0];if(!result||(typeof result==="undefined"?"undefined":_typeof(result))!='object'&&typeof result!='function'){throw new Error('extendee must be an object');}var extenders=args.slice(1);var len=extenders.length;for(var i=0;i<len;i++){var extender=extenders[i];for(var key in extender){// include prototype properties
var value=extender[key];if(deep&&value&&((typeof value==="undefined"?"undefined":_typeof(value))=='object'||typeof value=='function')){var base=Array.isArray(value)?[]:{};result[key]=extend(true,result[key]||base,value);}else{result[key]=value;}}}return result;}},{}],16:[function(require,module,exports){(function(global){"use strict";var userAgent=global.navigator&&global.navigator.userAgent;var isRunningInIE=userAgent&&userAgent.indexOf("MSIE ")>-1;var maxTimeout=Math.pow(2,31)-1;//see https://heycam.github.io/webidl/#abstract-opdef-converttoint
...
var NOOP=function NOOP(){return undefined;};var timeoutResult=setTimeout(NOOP,0);var addTimerReturnsObject=(typeof timeoutResult==="undefined"?"undefined":_typeof(timeoutResult))==="object";var hrtimePresent=global.process&&typeof global.process.hrtime==="function";var nextTickPresent=global.process&&typeof global.process.nextTick==="function";var performancePresent=global.performance&&typeof global.performance.now==="function";var requestAnimationFramePresent=global.requestAnimationFrame&&typeof global.requestAnimationFrame==="function";var cancelAnimationFramePresent=global.cancelAnimationFrame&&typeof global.cancelAnimationFrame==="function";clearTimeout(timeoutResult);var NativeDate=Date;var uniqueTimerId=1;/**
...
if((typeof timerId==="undefined"?"undefined":_typeof(timerId))==="object"){timerId=timerId.id;}if(clock.timers.hasOwnProperty(timerId)){// check that the ID matches a timer of the correct type
...
tickFrom+=clock.now-oldNow;tickTo+=clock.now-oldNow;}// perform each timer in the requested range
...
if(oldNow!==clock.now){tickFrom+=clock.now-oldNow;tickTo+=clock.now-oldNow;previous+=clock.now-oldNow;}}timer=firstTimerInRange(clock,previous,tickTo);previous=tickFrom;}// perform process.nextTick()s again
oldNow=clock.now;runJobs(clock);if(oldNow!==clock.now){// compensate for any setSystemTime() call during process.nextTick() callback
 ...
 * @param  {Object=}               options
 * @return {!RegExp}
 */function pathToRegexp(path,keys,options){if(!isarray(keys)){options=/** @type {!Object} */keys||options;keys=[];}options=options||{};if(path instanceof RegExp){return regexpToRegexp(path,/** @type {!Array} */keys);}if(isarray(path)){return arrayToRegexp(/** @type {!Array} */path,/** @type {!Array} */keys,options);}return stringToRegexp(/** @type {string} */path,/** @type {!Array} */keys,options);}},{"isarray":14}],18:[function(require,module,exports){(typeof define==="function"&&define.amd&&function(m){define("samsam",m);}||(typeof module==="undefined"?"undefined":_typeof(module))==="object"&&function(m){module.exports=m();}||// Node
...
return o.toString.call(value).split(/[ \]]/)[1];}/**
     ...
     * ``false`` otherwise.
     */function isArguments(object){if(getClass(object)==='Arguments'){return true;}if((typeof object==="undefined"?"undefined":_typeof(object))!=="object"||typeof object.length!=="number"||getClass(object)==="Array"){return false;}if(typeof object.callee=="function"){return true;}try{object[object.length]=6;delete object[object.length];}catch(e){return true;}return false;}/**
...
compared={};/**
         ...
         */function isObject(value){if((typeof value==="undefined"?"undefined":_typeof(value))==='object'&&value!==null&&!(value instanceof Boolean)&&!(value instanceof Date)&&!(value instanceof Number)&&!(value instanceof RegExp)&&!(value instanceof String)){return true;}return false;}/**
...
return function deepEqual(obj1,obj2,path1,path2){var type1=typeof obj1==="undefined"?"undefined":_typeof(obj1);var type2=typeof obj2==="undefined"?"undefined":_typeof(obj2);// == null also matches undefined
...
Object.defineProperty(TextDecoder.prototype,'ignoreBOM',{/** @this {TextDecoder} */get:function get(){return this._ignoreBOM;}});}/**
   ...
   */TextDecoder.prototype.decode=function decode(input,options){var bytes;if((typeof input==="undefined"?"undefined":_typeof(input))==='object'&&input instanceof ArrayBuffer){bytes=new Uint8Array(input);}else if((typeof input==="undefined"?"undefined":_typeof(input))==='object'&&'buffer'in input&&input.buffer instanceof ArrayBuffer){bytes=new Uint8Array(input.buffer,input.byteOffset,input.byteLength);}else{bytes=new Uint8Array(0);}options=ToDictionary(options);// 1. If the do not flush flag is unset, set decoder to a new
...
// is `undefined`, take a pure object instead
})(this||{});},{"./encoding-indexes.js":20}]},{},[12])(12);});