...
function getSourceCodeOfFile(filename, options) {
    ...
    var sourceCode = cli.linter.getSourceCode();
...
function getSourceCodeOfFiles(patterns, providedOptions, providedCallback) {
    ...
    var defaultOptions = Object.assign({}, baseDefaultOptions, { cwd: process.cwd() });
    ...
    if (typeof providedOptions === "undefined") {
        ...
        options = defaultOptions;
    } else if ((typeof providedOptions === "undefined" ? "undefined" : _typeof(providedOptions)) === "object") {
        options = Object.assign({}, defaultOptions, providedOptions);
        callback = providedCallback;
    }
    debug("constructed options:", options);
    var resolvedPatterns = globUtil.resolveFileGlobPatterns(globPatternsList, options);

    var filenames = globUtil.listFilesToProcess(resolvedPatterns, options).filter(function (fileInfo) {
        return !fileInfo.ignored;
    }).reduce(function (files, fileInfo) {
        return files.concat(fileInfo.filename);
    }, []);
