...
function findFile(cwd, name) {
    var ignoreFilePath = path.resolve(cwd, name);
...
var IgnoredPaths = function () {

    /**
     * @param {Object} providedOptions object containing 'ignore', 'ignorePath' and 'patterns' properties
     */
    function IgnoredPaths(providedOptions) {
        var _this = this;

        _classCallCheck(this, IgnoredPaths);

        var options = mergeDefaultOptions(providedOptions);
        ...
        if (options.ignore !== false) {
            ...
            if (ignorePath) {
                ...
                try {
                    ...
                    var packageJSONPath = findPackageJSONFile(options.cwd);

                    if (packageJSONPath) {
                        var packageJSONOptions = void 0;
                        ...
                        if (packageJSONOptions.eslintIgnore) {
                            if (Array.isArray(packageJSONOptions.eslintIgnore)) {
                                packageJSONOptions.eslintIgnore.forEach(function (pattern) {
                                    addPattern(_this.ig.custom, pattern);
                                    addPattern(_this.ig.default, pattern);
                                });
                            } else {
    ...
    _createClass(IgnoredPaths, [{
        key: "readIgnoreFile",
        value: function readIgnoreFile(filePath) {
        ...
        key: "addIgnoreFile",
        value: function addIgnoreFile(ig, filePath) {
        ...
        key: "contains",
        value: function contains(filepath, category) {

            var result = false;
            var absolutePath = path.resolve(this.options.cwd, filepath);
            var relativePath = pathUtil.getRelativePath(absolutePath, this.baseDir);
        ...
        key: "getIgnoredFoldersGlobChecker",
        value: function getIgnoredFoldersGlobChecker() {
