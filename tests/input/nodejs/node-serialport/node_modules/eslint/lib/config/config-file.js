...
function configMissingError(configName) {
    var error = new Error("Failed to load config \"" + configName + "\" to extend from.");
...
function applyExtends(config, configContext, filePath, relativeTo) {
    ...
    return configExtends.reduceRight(function (previousValue, parentPath) {
        try {
            var extensionPath = void 0;
            ...
            if (parentPath.startsWith("eslint:")) {
                 ...
                 */
                extensionPath = path.isAbsolute(parentPath) ? parentPath : path.join(relativeTo || path.dirname(filePath), parentPath);
            } else {
...
function resolve(filePath, relativeTo) {
    if (isFilePath(filePath)) {
        var fullPath = path.resolve(relativeTo || "", filePath);
    ...
    if (filePath.startsWith("plugin:")) {
        var configFullName = filePath;
        var pluginName = filePath.slice(7, filePath.lastIndexOf("/"));
        var configName = filePath.slice(filePath.lastIndexOf("/") + 1);
...
function loadFromDisk(resolvedPath, configContext) {
    var dirname = path.dirname(resolvedPath.filePath),
    ...
    if (config) {
        ...
        var ruleMap = configContext.linterContext.getRules();
...
function load(filePath, configContext, relativeTo) {
    ...
    var cachedConfig = configContext.configCache.getConfig(resolvedPath.configFullName);

    if (cachedConfig) {
        return cachedConfig;
    }

    var config = loadFromDisk(resolvedPath, configContext);
