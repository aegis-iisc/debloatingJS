...
describe('add-stream', function () {
	...
	describe('buffer mode', function () {
		it('should append a stream', function (done) {
			...
			emit(firstChunks).pipe(addStream(emit(secondChunks))).pipe(es.wait(function (err, buffer) {
		...
		it('should append a stream from a factory function', function (done) {
			var firstChunks = ['abc', 'def'];
			var secondChunks = ['ghi', 'jkl'];
			emit(firstChunks).pipe(addStream(function () {
	...
	describe('object mode', function () {
		it('should append a stream', function (done) {
			es.readArray([{ p: 1 }, { p: 2 }, { p: 3 }]).pipe(addStream.obj(es.readArray([{ p: 4 }, { p: 5 }, { p: 6 }]))).pipe(es.writeArray(function (err, array) {
		...
		it('should append a stream from a factory function', function (done) {
			es.readArray([{ p: 1 }, { p: 2 }, { p: 3 }]).pipe(addStream.obj(function () {
				return es.readArray([{ p: 4 }, { p: 5 }, { p: 6 }]);
