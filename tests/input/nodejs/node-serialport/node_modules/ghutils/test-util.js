...
function makeServer(data) {
  var ee = new EE(),
    ...
    if (_data && _data.headers && _data.headers.link) res.setHeader('link', _data.headers.link);

    res.end(JSON.stringify(_data && _data.response || _data));

    if (!Array.isArray(data) || i == data.length) server.close();
...
function verifyUrl(t, urls) {
  ...
  return function (_url) {
    if (i == urls.length) return t.fail('too many urls/requests');
