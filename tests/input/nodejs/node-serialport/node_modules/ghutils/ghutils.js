...
function makeOptions(auth, options) {
  var headers = xtend({ 'user-agent': 'Magic Node.js application that does magic things' }, (typeof options === 'undefined' ? 'undefined' : _typeof(options)) == 'object' && _typeof(options.headers) == 'object' ? options.headers : {});
...
function handler(callback) {
  return function responseHandler(err, data, res) {
    ...
    if (data && (data.error || data.message)) return callback(new Error('Error from GitHub: ' + (data.error || data.message)));
...
function lister(auth, urlbase, options, callback) {
  ...
  optqs = qs.stringify(optqsobj);(function next(url) {
    ...
    ghget(auth, url, options, function (err, data, res) {
      ...
      if (data.length) retdata.push.apply(retdata, data);
