...
function conventionalChangelogWriter(context, options) {
  ...
  options = _.assign({
    ...
    notesSort: 'text',
    generateOn: function generateOn(commit) {
      return semverValid(commit.version);
    },
    finalizeContext: function finalizeContext(context) {
      return context;
    },
  ...
  if (!_.isFunction(options.transform) && _.isObject(options.transform) || _.isUndefined(options.transform)) {
    options.transform = _.assign({
      hash: function hash(_hash) {
        if (_.isString(_hash)) {
          return _hash.substring(0, 7);
        }
      },
      header: function header(_header) {
        return _header.substring(0, 100);
      },
      committerDate: function committerDate(date) {
        if (!date) {
          return;
        }
  ...
  if (_.isString(generateOn)) {
    ...
    generateOn = function generateOn() {
      return false;
    };
  }
