...
module.exports = {
  ...
  create: function create(context) {
    ...
    function reportNoBeginningSpace(node, token) {
      context.report(node, token.loc.start, "There should be no space after '" + token.value + "'");
    }
    ...
    function reportNoEndingSpace(node, token) {
      context.report(node, token.loc.start, "There should be no space before '" + token.value + "'");
    }
    ...
    function reportRequiredBeginningSpace(node, token) {
      context.report(node, token.loc.start, "A space is required after '" + token.value + "'");
    }
    ...
    function reportRequiredEndingSpace(node, token) {
      context.report(node, token.loc.start, "A space is required before '" + token.value + "'");
    }
    ...
    function validateArraySpacing(node) {
      ...
      var openingBracketMustBeSpaced = options.objectsInArraysException && second.value === '{' || options.arraysInArraysException && second.value === '[' || options.singleElementException && node.elements.length === 1 ? !options.spaced : options.spaced;

      var closingBracketMustBeSpaced = options.objectsInArraysException && penultimate.value === '}' || options.arraysInArraysException && penultimate.value === ']' || options.singleElementException && node.elements.length === 1 ? !options.spaced : options.spaced;
