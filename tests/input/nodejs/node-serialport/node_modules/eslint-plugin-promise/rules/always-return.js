...
function isThenCallExpression(node) {
  return node.type === 'CallExpression' && node.callee.type === 'MemberExpression' && node.callee.property.name === 'then';
}

function isFirstArgument(node) {
  return node.parent && node.parent.arguments && node.parent.arguments[0] === node;
}

function isInlineThenFunctionExpression(node) {
  return isFunctionWithBlockStatement(node) && isThenCallExpression(node.parent) && isFirstArgument(node);
}
...
module.exports = {
  ...
  create: function create(context) {
    ...
    return {
      ...
      onCodePathSegmentStart: function onCodePathSegmentStart(segment, node) {
        var funcInfo = peek(funcInfoStack);
        funcInfo.branchIDStack.push(segment.id);
        funcInfo.branchInfoMap[segment.id] = { good: false, node: node };
      },

      onCodePathSegmentEnd: function onCodePathSegmentEnd(segment, node) {
        var funcInfo = peek(funcInfoStack);
        funcInfo.branchIDStack.pop();
      },

      onCodePathStart: function onCodePathStart(path, node) {
        funcInfoStack.push({
          branchIDStack: [],
          branchInfoMap: {}
        });
      ...
      onCodePathEnd: function onCodePathEnd(path, node) {
        ...
        path.finalSegments.forEach(function (segment) {
          var id = segment.id;
