...
test('strip + map', function (t) {
  ...
  tar.pack(a).pipe(tar.extract(b, { strip: 1, map: uppercase })).on('finish', function () {
    var files = fs.readdirSync(b).sort();
    t.same(files.length, 1);
    t.same(files[0], 'TEST.TXT');
  });
...
test('finish callbacks', function (t) {
  ...
  pack.pipe(tar.extract(b, { map: countExtractEntry, finish: onExtractFinish })).on('finish', function () {
    t.end();
  });
});
...
test('not finalizing the pack', function (t) {
  ...
  function packB(pack) {
    tar.pack(b, { pack: pack, map: prefixer('b-files') }).pipe(tar.extract(out)).on('finish', assertResults);
  }
